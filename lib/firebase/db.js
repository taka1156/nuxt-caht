...
const TIME_STAMP = db.FieldValue.serverTimestamp();
const { Timestamp } = db;
...
const postRoom = async room => {
  ...
  const result = await roomDocRef
    ...
    .then(async () => {
      ...
      const botImg =
        'https://firebasestorage.googleapis.com/v0/b/chatapp-f1e5d.appspot.com/o/bot.png?alt=media&token=72193d5d-c36c-459a-86f8-f503067e0a05';
...
const getRooms = async ({ seconds, nanoseconds }) => {
  ...
  // 5件ずつ取得
  const rooms = await sortRoom.limit(5);
...
export { getRoom, getRooms, postRoom, getChats, postChat };
